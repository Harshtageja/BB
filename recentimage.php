...
<body>
  ...
  <div class="collapse navbar-collapse" id="navbarNavAltMarkup" style="text-align: right;background-color: black">
   ...
   <a class="nav-link active" href="#" style="margin-left: 300px;"onMouseOver="this.style.color='yellow'"
	...
	$sql="select image,id from images1 where Email='".$_POST['Email']."'";
	$res=$conn->query($sql);
	if($res->num_rows>0){?>
<div class="row">
    ...
    while($row=$res->fetch_assoc()){
    	?>
    	<form method="post" action='http://localhost/recentimage1.php'>
  <div class="col-sm-6">
		<div class="card" style="width: 15rem;height: 300px;margin: 50px 50px;padding: 15px 15px;">

  <img src="data:image/jpg;charset=utf8;base64,<?php echo base64_encode($row['image']);?>" class="card-img-top" style="width: 200px;height: 200px;">
  <div class="card-body">
  	<input readonly type="text" name="id" style="display: none" value="<?php echo $row['id'] ?>" >
  	<input readonly type="text" name="Email" style="display: none" value="<?php echo $_POST['Email'] ?>" >

    <button type="submit" class="btn btn-primary" style="background-color: black">Change Image</button>
  </div>
</div>
</div>
</form>
