...
<body>
  ...
  $sql="select id,Email from driver where verify IS NULL";
  $res=$conn->query($sql);
  if($res->num_rows>0){
    ...
    while($row=$res->fetch_assoc()){
      echo $row['Email'];
      ?>
      <form method="post" action="verifywannadrive2.php">
        <input type="text" name="id" value="<?php echo $row['id'] ?>">
        <button type="submit" class="b">continue</button></form><br><br><br><br>
      </form>
<?php
  }
  ?>
